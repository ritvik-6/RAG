...
# Import isolated modular workers
...
def create_orchestrator_agent(user_id: str, collection_name: str):
    """Compiles the parent supervisor engine with decoupled tools."""

    @tool
    def rag_sub_agent(query: str) -> str:
        """Call this agent tool strictly to search, extract, and read text insights
        from within the user's uploaded physical document scopes."""
        return run_rag_sub_agent(query, collection_name)

    @tool
    async def catalog_sub_agent(query: str) -> str:
        """Call this agent tool immediately if the user is checking document status,
    ...
    # Return the LangChain orchestrator loop
